        ...
        VStack(alignment: .leading) {
            Group {
                VStack(alignment: .leading) {
                    ...
                    ContentLabel(content: viewModel.transaction.date)
                }
                VStack(alignment: .leading) {
                    TitleLabel(title: "ORDER NUMBER")
                    ContentLabel(content: viewModel.transaction.orderNumber)
                }
                VStack(alignment: .leading) {
                    TitleLabel(title: "MONEY CHANGER")
                    ContentLabel(content: viewModel.transaction.moneyChangerName)
                }
                VStack(alignment: .leading) {
                    TitleLabel(title: "ADDRESS")
                    ContentLabel(content: viewModel.transaction.moneyChangerAddress)
                }
                VStack(alignment: .leading) {
                    TitleLabel(title: "TIME")
                    ContentLabel(content: viewModel.transaction.time)
                }
                VStack(alignment: .leading) {
                    TitleLabel(title: "TRANSACTION")
                    ContentLabel(content: viewModel.transaction.amount)
                }
            }.padding(.top)
